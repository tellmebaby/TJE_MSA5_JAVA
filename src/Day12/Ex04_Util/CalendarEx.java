...
public class CalendarEx {
	...
	public static void printCalendar(String msg, Calendar cal) {
		...
		int month = cal.get( Calendar.MONTH ) + 1;			// 월(0~11) (+1을 해야 1~12가 됨)
	...
	public static void main(String[] args) {
		...
		System.out.println("1970년 1월 1일 0시 0분 0ms ~ 오늘까지 일 수 : " + nowD);
		System.out.println("1970년 1월 1일 0시 0분 0ms ~ 종강까지 일 수 : " + lastD);
		
		// 내가 태어난 지 몇 일
		Calendar birth = Calendar.getInstance();
		birth.set(2000, Calendar.APRIL, 15, 0, 0, 0);			// 생일 지정
		long birthD = birth.getTimeInMillis() / (1000 * 60 * 60 * 24);
		long afterBirth = nowD - birthD;
		System.out.println("생후 " + afterBirth + "일이 지났습니다.");
		
		// * 월(MONTH)는 0~11 로 0이 1월을 의미하므로, 1월이면 0으로 지정해야 한다.
		...
		if( now.before(lastDay) ) {
			...
			System.out.println("수업이 종료된 강의 입니다.");
		}
	}
}
